...
В контексте Angular, ChangeDetectionStrategy.OnPush — это стратегия обнаружения изменений, которая оптимизирует производительность. <br>
По умолчанию Angular использует стратегию `ChangeDetectionStrategy.Default`, которая проверяет изменения в компоненте и его потомках при каждом изменении данных.
<b>Стратегия OnPush позволяет Angular проверять изменения только в тех случаях, когда:</b><br><br>
- Изменяются входные данные компонента (inputs). Например, если объект или массив был передан через @Input(), и его ссылка изменилась, Angular выполнит проверку.
- Событие (event) внутри компонента, связанное с пользовательским вводом.
- Вручную вызвано изменение детекции, например, через ChangeDetectorRef.markForCheck() или ChangeDetectorRef.detectChanges().
<br>
...
<br>
- A zone is an execution context that persists across async tasks. You can think of it as thread-local storage for the JavaScript VM<br><br>
<b>Immutable object</b> — это объект, состояние которого нельзя изменить после его создания.
<br>То есть, после того как объект был инициализирован, его поля или свойства остаются неизменными (immutable). Это делает объекты таким образом предсказуемыми и безопасными в многозадачных приложениях, поскольку исключаются неожиданные изменения данных.
<br>Примеры immutable objects включают строки в Java, числа в JavaScript или объекты, созданные с использованием Object.freeze() в JavaScript.
